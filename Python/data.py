    ...
    while True:
        ...
        if pictures is not None:
            for picture in pictures.findall('./picture'):
                ...
                cursor = db.cursor(pymysql.cursors.DictCursor)
                # try:
                cursor.execute(query=sql, args=(picture.find('hotel_id').text, picture.find('picture_id').text, picture.find(
                    'caption').text, picture.find('caption_translated').text, picture.find('URL').text))
                db.commit()
                # except:
                #     pass
                s += 1
        ...
        if roomtypes is not None:
            for roomtype in roomtypes.findall('./roomtype'):
                ...
                if roomtype.find('shared_bathroom').text == 'True':
                    ...
                    shared_bathroom = 0
                # try:
                cursor.execute(query=sql, args=(roomtype.find('hotel_id').text, roomtype.find('hotel_room_type_id').text, roomtype.find('max_occupancy_per_room').text, roomtype.find('no_of_room').text, roomtype.find('size_of_room').text, roomtype.find('max_extrabeds').text, roomtype.find('max_infant_in_room').text, roomtype.find(
                    'hotel_master_room_type_id').text, roomtype.find('standard_caption').text, roomtype.find('standard_caption_translated').text, roomtype.find('views').text, roomtype.find('hotel_room_type_picture').text, roomtype.find('bed_type').text, roomtype.find('room_size_incl_terrace').text, shared_bathroom, roomtype.find('gender').text))
                db.commit()
                # except:
                #     pass
                s += 1
    ...
    while True:
        ...
        if facilities is not None:
            for facility in facilities.findall('./facility'):
                ...
                # try:
                cursor.execute(query=sql, args=(facility.find('hotel_id').text, facility.find('property_id').text, facility.find(
                    'property_group_description').text, facility.find('property_name').text, facility.find('property_translated_name').text))
                db.commit()
                # except:
                #     pass
                s += 1
    ...
    while True:
        ...
        if hotelinfos is not None:
            for hotelinfo in hotelinfos.findall('./hotelinfo'):
                ...
                # try:
                cursor.execute(query=sql, args=(hotelinfo.find('hotel_id').text, hotelinfo.find('property_id').text, hotelinfo.find(
                    'property_name').text, hotelinfo.find('property_translated_name').text, hotelinfo.find('property_details').text))
                db.commit()
                # except:
                #     pass
                s += 1
...
def saveRoomTypeFacilities():
    ...
    while True:
        ...
        roomtype_facilities = Roomtype_facility_feed.find(
            'roomtype_facilities')
        if roomtype_facilities is not None:
            for roomtype_facility in roomtype_facilities.findall('./roomtype_facility'):
                ...
                # try:
                cursor.execute(query=sql, args=(roomtype_facility.find('hotel_id').text, roomtype_facility.find('property_id').text, roomtype_facility.find(
                    'property_name').text, roomtype_facility.find('property_translated_name').text, roomtype_facility.find('property_details').text))
                db.commit()
                # except:
                #     pass
                s += 1
    ...
    while True:
        ...
        if hotel_addresses is not None:
            for hotel_address in hotel_addresses.findall('./hotel_address'):
                ...
                # try:
                cursor.execute(query=sql, args=(hotel_address.find('hotel_id').text, hotel_address.find('address_type').text, hotel_address.find('address_line_1').text, hotel_address.find(
                    'address_line_2').text, hotel_address.find('postal_code').text, hotel_address.find('state').text, hotel_address.find('city').text, hotel_address.find('country').text))
                db.commit()
                # except:
                #     pass
                s += 1
