...
def saveCities():
    data = requests.get(
        f'http://affiliatefeed.agoda.com/datafeeds/feed/getfeed?apikey=d3beca16-7cb7-490f-94e5-11c0a076dcf5&feed_id=3&ocountry_id=212&olanguage_id=9')
    City_feed = elemTree.fromstring(data.text)
    cities = City_feed.find('cities')
    if cities is not None:
        for city in cities.findall('./city'):
            if city.find('city_id').text == '3818' or city.find('city_id').text == '10137':
                continue
            saveHotels(city.find('city_id').text)
...
def saveHotels(counter):
    global found
    print(f'city id: {counter}')
    s = 0;
    data = requests.get(
    ...
    if hotels is not None:
        for hotel in hotels.findall('./hotel'):
            if hotel.find('hotel_id').text == '37767700':
                found = True
            if found:
                saveFull(hotel.find('hotel_id').text)
...
def saveFull(counter):
    print(f'hotel id : {counter}')
    data = requests.get(
        f'http://affiliatefeed.agoda.com/datafeeds/feed/getfeed?apikey=d3beca16-7cb7-490f-94e5-11c0a076dcf5&mhotel_id={counter}&feed_id=19&olanguage=9')
